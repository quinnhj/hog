...
void image_to_hist_parallel5 (cl_mem &image, cl_mem &hist, int width, int height,
    ...
    err = clSetKernelArg(kernel, 10, sizeof(int), &by);
    CHK_ERR(err);
...
void hist_to_blocks_parallel3(cl_mem &hist, cl_mem &normalised_blocks, int by, int bx,
    ...
    int n_cellsx_num = n_cellsx * num_orientations;
    ...
    err = clSetKernelArg(kernel, 6, sizeof(int), &num_orientations);
    CHK_ERR(err);

    err = clSetKernelArg(kernel, 7, sizeof(int), &n_cellsx_num);
    CHK_ERR(err);
...
void hist_to_blocks_parallel5(cl_mem &hist, cl_mem &normalised_blocks, int by, int bx,
    ...
    err = clSetKernelArg(kernel, 5, sizeof(int), &by);
    CHK_ERR(err);

    err = clSetKernelArg(kernel, 6, sizeof(int), &n_blocksx);
    CHK_ERR(err);

    err = clSetKernelArg(kernel, 7, sizeof(int), &num_orientations);
    CHK_ERR(err);

    err = clSetKernelArg(kernel, 8, sizeof(int), &n_cellsx_num);
    CHK_ERR(err);
